...
let moveLeft = false; //declare variable moveLeft and initialize it to false
let moveRight = false; //declare variable moveRight and initialize it to false
...
document.addEventListener('keydown', function(event) {
  //set up an event listener in JavaScript that listens for the keydown event on the entire HTML document

  //the if statement checks if the key that was pressed is the left arrow key ('ArrowLeft').
  if (event.key === 'ArrowLeft') { //Event Object: what key was pressed?
    moveLeft = true; //sets the moveLeft boolean variable to true.
    
    //When the ArrowLeft key is pressed,

  } else if (event.key === 'ArrowRight') { //Event Object: what key was pressed?
    moveRight = true; //sets the moveRight boolean variable to true.
